 ...
 * @returns {{ email: string, password: string, proxy: string|null } | null} 解析失敗回傳 null
 */
const parseAccountLine = (line) => {
  ...
  // 先按第一個 '|' 切出可選 proxy（proxy 部分自身可能含有 '|'，例如 query 參數極少見，這裡按首個分隔）
