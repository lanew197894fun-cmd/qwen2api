...
/**
 * 從完整MIME類型取得簡化的檔案類型
 * @param {string} mimeType - 完整的MIME類型
 * @returns {string} 簡化檔案類型
 */
 ...
 * @param {number} filesize - 檔案大小（字節）
 * @param {string} filetypeSimple - 簡化檔案類型
 * @param {string} authToken - 認證Token
 * @param {number} retryCount - 重試次數
 * @param {Object} [account] - 賬戶物件（用於解析帳號級代理）
 * @returns {Promise<Object>} STS Token回應資料
 */
const requestStsToken = async (filename, filesize, filetypeSimple, authToken, retryCount = 0, account) => {
    try {
        // 參數驗證
        if (!filename || !authToken) {
            logger.error('檔案名和認證Token不能為空', 'UPLOAD')
            throw new Error('檔案名和認證Token不能為空')
        }
        ...
        if (response.status === 200 && response.data) {
            ...
            if (missingCredentials.length > 0 || missingFileInfo.length > 0) {
                logger.error(`STS回應資料不完整: 缺少 ${[...missingCredentials, ...missingFileInfo].join(', ')}`, 'UPLOAD')
                throw new Error(`STS回應資料不完整: 缺少 ${[...missingCredentials, ...missingFileInfo].join(', ')}`)
            }

            logger.success('STS Token取得成功', 'UPLOAD')
            return { credentials, file_info: fileInfo }
        } else {
            logger.error(`取得STS Token失敗，狀態碼: ${response.status}`, 'UPLOAD')
            throw new Error(`取得STS Token失敗，狀態碼: ${response.status}`)
        }
    } catch (error) {
        ...
        if (error.response?.status === 403) {
            logger.error('403 Forbidden錯誤，可能是Token權限問題', 'UPLOAD')
            logger.error('認證失敗，請檢查Token權限', 'UPLOAD')
            throw new Error('認證失敗，請檢查Token權限')
        }
 ...
 * @param {string} fileContentTypeFull - 檔案的完整MIME類型
 * @param {number} retryCount - 重試次數
...
const uploadToOssWithSts = async (fileBuffer, stsCredentials, ossInfo, fileContentTypeFull, retryCount = 0) => {
    try {
        ...
        if (!fileBuffer || !stsCredentials || !ossInfo) {
            logger.error('缺少必要的上傳參數', 'UPLOAD')
            throw new Error('缺少必要的上傳參數')
        }
...
/**
 * 完整的檔案上傳流程：取得STS Token -> 上傳到OSS。
 * @param {Buffer} fileBuffer - 圖片檔案的Buffer。
 * @param {string} originalFilename - 原始檔案名 (例如 "image.png")。
