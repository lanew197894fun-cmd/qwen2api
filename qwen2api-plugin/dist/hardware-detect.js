...
const getEnvType = () => {
    ...
    try {
        if (isWin) {
            const out = execSync("wmic path Win32_Battery get BatteryStatus /format:csv 2>nul", { timeout: 3000, encoding: "utf8" });
            ...
            const bats = execSync("ls /sys/class/power_supply/BAT*/status 2>/dev/null", {
                timeout: 3000,
                encoding: "utf8",
            })
                .trim()
            ...
            if (bats.length > 0) {
                isLaptop = true;
                const st = execSync("cat /sys/class/power_supply/BAT*/status 2>/dev/null", {
                    timeout: 3000,
                    encoding: "utf8",
                }).trim();
...
const getGpuInfo = () => {
    ...
    try {
        if (isWin) {
            const out = execSync("wmic path win32_VideoController get name /format:csv 2>nul", { timeout: 3000, encoding: "utf8" });
    ...
    if (gpu?.includes("NVIDIA")) {
        try {
            const nvidia = execSync(isWin
                ? "nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits"
                : "nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits 2>/dev/null", { timeout: 3000, encoding: "utf8" });
...
const getLoadInfo = () => {
    ...
    try {
        if (isWin) {
            ...
            const out = execSync("wmic path Win32_Processor get LoadPercentage /format:csv 2>nul", { timeout: 3000, encoding: "utf8" });
